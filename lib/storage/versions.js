...
function getData(endpointUrl, versionNum, cb) {
  // TODO: accepts versionNum to fetch data from a
  // particular version, not just the latest

  var name = utils.endpointNameFromPath(endpointUrl)

  current(endpointUrl, function(err, num) {
    if (err) { cb(err); return }

    var filePath = utils.dataPath + name + '/' + num
