...
module.exports = {
  ...
  rollback: function(endpoint) {
    if (!endpoint) {
      console.log('missing endpoint. copy it from the list command')
      return
    }
    ...
    storage.versions.current(endpoint, function(err, num) {
      ...
      storage.versions.remove(endpoint, num, function(err) {
        if (err) { console.log('error rolling back', err) }
      })
    })
  },
  ...
  remove: function(endpoint) {
    ...
    prompt.get(schema, function (err, result) {
      if (err) { console.log('invalid response'); return }

      if (result.confirm == 'yes' || result.confirm == 'y') {
        storage.endpoints.remove(endpoint, function(err) {
          if (err) { console.log('endpoint not found'); return }
